...
public class UI_CharacterDescription : UI_Panel 
{
    ...
    {
        ui_nameText.text = playerCharacter.Name;
        ui_levelText.text = _profileManager.CurrentProfile.CharacterManager.GetCharacterLevel().ToString();

        ui_stats.SetStats((PlayerCharacterStats)playerCharacter.Stats);
        ui_stats.UpdateUI();

        ui_spellsList.SetInfo(playerCharacter.StartItems.GetStartSpells());
    }
